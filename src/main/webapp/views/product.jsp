...
<body>
<c:if test="${!empty product.name}"><h1>Edit Product</h1></c:if>
	<c:if test="${empty product.name}"><h1>Add a Product</h1></c:if>
	...
	<form:form action="${addAction}" commandName="product" method="post"
		...
		<table>
			...
			<td><img src="<c:url value="/resources/images/${product.id}.jpg" />"
							alt="image" style="width: 30%" />
				<form:label path="productImage">
						<spring:message text="Change Image" />
					</form:label></td>
					</c:if>
					<c:if test="${empty product.name}">
				<td><form:label path="productImage">
